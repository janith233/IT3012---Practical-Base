...
class SearchAgent:
    ...
    def check_tile_feasibility(self, tile, percept):
        ...
        # Get tile percept information
        target_visible_positions = percept.get(
            'target_visible_positions', []
        )

        dust_positions = percept.get(
            'dust_positions', []
        )

        bloodseeker_missing_positions = percept.get(
            'bloodseeker_missing_positions', []
        )

        # Add current tile facts
        if tile in target_visible_positions:
            self.kb.tell_fact('TargetVisible')

        if tile in dust_positions:
            self.kb.tell_fact('HasDust')

        if tile in bloodseeker_missing_positions:
            self.kb.tell_fact('BloodseekerMissing')

        # Run inference
        self.kb.forward_chain()

        # Check if tile is feasible
